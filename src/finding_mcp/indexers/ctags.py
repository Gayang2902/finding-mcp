...
from ..core.languages import CTAGS_LANGUAGE
...
# Cache: project_root_str -> (cache_key, CtagsIndex)
# Only the latest commit per project is kept in memory.
_INDEX_CACHE: dict[str, tuple[str, CtagsIndex]] = {}
...
def get_index(project_root: Path, cache_dir: Path) -> CtagsIndex:
    ...
    current_key = cache_key(project_root)

    with _CACHE_LOCK:
        cached = _INDEX_CACHE.get(root_str)
        if cached is not None and cached[0] == current_key:
            return cached[1]

    tags_file = cache_dir / "tags" / f"{current_key}-{abs(hash(root_str))}.json"
    ...
    with _CACHE_LOCK:
        _INDEX_CACHE[root_str] = (current_key, index)
    return index
