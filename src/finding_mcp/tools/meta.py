...
from ..core.git_utils import cache_key, get_head_commit, is_working_tree_dirty
...
def get_repo_info(settings: Settings) -> RepoInfo:
    ...
    return RepoInfo(
        ...
        commit_hash=get_head_commit(root),
        is_dirty=is_working_tree_dirty(root),
...
def register(mcp: FastMCP, settings: Settings) -> None:
    ...
    def _list_files(glob: str | None = None, limit: int = 200) -> dict:
        """List files in the project, optionally filtered by glob."""
        result = list_files(settings, glob, limit)
    ...
    def _get_project_structure(max_depth: int = 3, include_file_sizes: bool = False) -> dict:
        ...
        """
        return get_project_structure(settings, max_depth, include_file_sizes)
